...
// Données : Sur-mesure (highlight) est bien en 2ème position
const pricingPlans = [
  {
    id: 'vitrine',
    name: 'Site Vitrine',
...
export default function TarifsPage() {
  ...
  useGSAP(() => {
    ...
    // On utilise .fromTo() au lieu de .from()
    // Cela force l'état final à opacity: 1 et y: 0, même si le CSS dit autre chose.
    tl.fromTo('.tarifs-card-wrapper', 
      { 
        y: 60, 
        opacity: 0 // État de départ
      },
      {
        y: 0,
        opacity: 1, // État d'arrivée FORCÉ
        duration: 0.8,
        ease: 'power3.out',
        stagger: 0.15
      },
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto mb-20">
          {pricingPlans.map((plan, index) => (
            // WRAPPER ANIMÉ PAR GSAP
            // opacity-0 pour éviter le flash, mais fromTo va le corriger immédiatement
            <div 
              key={plan.id} 
              className="tarifs-card-wrapper opacity-0" 
            >
              {/* ENFANT GÉRÉ PAR FRAMER MOTION (Hover/Click) */}
              <motion.div
                className={`relative h-full rounded-2xl border transition-all duration-500 cursor-pointer flex flex-col ${
                  plan.highlight
                    ? 'border-arti-black bg-arti-black text-white shadow-2xl md:scale-105 z-10'
              ...
              >
                {plan.highlight && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-white text-arti-black text-sm font-medium rounded-full whitespace-nowrap shadow-sm">
                    Le plus populaire
                  </div>
                )}
                ...
                <div className="p-8 md:p-10 flex-grow">
                  ...
                  <Link
                    href="/contact"
                    className={`block w-full text-center px-6 py-4 rounded-full font-medium transition-all duration-300 mt-auto ${
                      plan.highlight
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b-2 border-arti-black">
                  ...
                  <th className="text-center py-4 px-4 font-medium bg-arti-black/5">Sur-mesure</th>
                  <th className="text-center py-4 px-4 font-medium">E-commerce</th>
                </tr>
              </thead>
              <tbody className="text-sm">
                <tr className="border-b border-gray-200">
                  ...
                  <td className="text-center py-4 px-4 bg-arti-black/5">Illimité</td>
                  <td className="text-center py-4 px-4">10-30</td>
                </tr>
                <tr className="border-b border-gray-200">
                ...
                <tr className="border-b border-gray-200">
                  ...
                  <td className="text-center py-4 px-4">Basique</td>
                  <td className="text-center py-4 px-4 bg-arti-black/5">Expert</td>
                  <td className="text-center py-4 px-4">Avancé</td>
                </tr>
                <tr className="border-b border-gray-200">
                  <td className="py-4 px-4">Support gratuit</td>
                  <td className="text-center py-4 px-4">1 mois</td>
                  <td className="text-center py-4 px-4 bg-arti-black/5">6 mois</td>
                  <td className="text-center py-4 px-4">2 mois</td>
                </tr>
              </tbody>
            </table>
          </div>
