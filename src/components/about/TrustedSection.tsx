'use client'
import { useRef, useState, useEffect } from 'react'
import Image from 'next/image'
import Link from 'next/link'
import gsap from 'gsap'
import { useGSAP } from '@gsap/react'

// --- DONNÉES ---
const projects = [
  {
    id: 1,
    client: "Charit.io",
    category: "Brand Identity",
    cover: "/projects/Charitio1.png", // Vérifiez vos chemins d'images
    gallery: ["/projects/Charitio1.png", "/projects/Tautu2.png", "/projects/Tautu1.png"],
    fullWidth: false 
  },
  {
    id: 2,
    client: "Keleti Design",
    category: "Mobile Design",
    cover: "/projects/Tautu1.png",
    gallery: ["/projects/Tautu1.png", "/projects/Tautu2.png", "/projects/Charitio1.png"],
    fullWidth: false 
  },
  {
    id: 3,
    client: "Disobey",
    category: "Brand Identity",
    cover: "/projects/Tautu2.png",
    gallery: ["/projects/Tautu2.png", "/projects/Charitio1.png", "/projects/Tautu1.png"],
    fullWidth: false
  },
  {
    id: 4,
    client: "Lumyn",
    category: "Web Marketing",
    cover: "/projects/Charitio1.png",
    gallery: ["/projects/Charitio1.png", "/projects/Tautu2.png", "/projects/Tautu1.png"],
    fullWidth: false
  }
]

// --- SOUS-COMPOSANT CARTE ---
const ProjectCard = ({ project }: { project: typeof projects[0] }) => {
  const containerRef = useRef<HTMLAnchorElement>(null)
  const cursorRef = useRef<HTMLDivElement>(null)
  
  const xTo = useRef<any>(null)
  const yTo = useRef<any>(null)
  
  const [galleryIndex, setGalleryIndex] = useState(0)
  const [isHovered, setIsHovered] = useState(false)

  const CARD_WIDTH = 280;
  const CARD_HEIGHT = 360;
  const PADDING = 20;

  useGSAP(() => {
    if (cursorRef.current) {
      xTo.current = gsap.quickTo(cursorRef.current, "left", { duration: 0.3, ease: "power3.out" })
      yTo.current = gsap.quickTo(cursorRef.current, "top", { duration: 0.3, ease: "power3.out" })
    }
  }, { scope: containerRef })

  useEffect(() => {
    let interval: NodeJS.Timeout
    if (isHovered) {
      interval = setInterval(() => {
        setGalleryIndex((prev) => (prev + 1) % project.gallery.length)
      }, 550)
    } else {
      setGalleryIndex(0)
    }
    return () => clearInterval(interval)
  }, [isHovered, project.gallery.length])

  const handleMouseMove = (e: React.MouseEvent) => {
    if (!containerRef.current || !xTo.current || !yTo.current) return
    const rect = containerRef.current.getBoundingClientRect()
    const mouseX = e.clientX - rect.left
    const mouseY = e.clientY - rect.top

    const minX = (CARD_WIDTH / 2) + PADDING
    const maxX = rect.width - (CARD_WIDTH / 2) - PADDING
    const minY = (CARD_HEIGHT / 2) + PADDING
    const maxY = rect.height - (CARD_HEIGHT / 2) - PADDING

    xTo.current(Math.max(minX, Math.min(mouseX, maxX)))
    yTo.current(Math.max(minY, Math.min(mouseY, maxY)))
  }

  const handleMouseEnter = () => {
    setIsHovered(true)
    gsap.to(cursorRef.current, { scale: 1, opacity: 1, duration: 0.3, ease: "back.out(1.2)" })
  }

  const handleMouseLeave = () => {
    setIsHovered(false)
    gsap.to(cursorRef.current, { scale: 0, opacity: 0, duration: 0.3, ease: "power2.in" })
  }

  return (
    <div className="flex flex-col gap-4 col-span-1">
      <Link 
        href={`/works/${project.id}`}
        ref={containerRef} 
        className="relative w-full h-[400px] md:h-[500px] overflow-hidden bg-gray-100 cursor-none block group"
        onMouseMove={handleMouseMove}
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <Image 
          src={project.cover}
          alt={project.client}
          fill
          className="object-cover transition-transform duration-700 group-hover:scale-105"
        />
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors duration-500 ease-out z-10" />

        {/* Carte Flottante */}
        <div 
          ref={cursorRef}
          className="absolute pointer-events-none z-20 opacity-0 scale-0 origin-center"
          style={{ width: CARD_WIDTH, height: CARD_HEIGHT, transform: 'translate(-50%, -50%)' }}
        >
          <div className="relative w-full h-full bg-white shadow-2xl overflow-hidden">
            <div className="relative w-full h-full bg-gray-200">
               <Image
                 src={project.gallery[galleryIndex]}
                 alt="Gallery"
                 fill
                 className="object-cover"
               />
            </div>
            <div className="absolute top-4 left-0 w-full text-center z-10">
               <span className="text-white mix-blend-difference text-[10px] uppercase tracking-widest font-bold">
                 Étude de cas {project.client}
               </span>
            </div>
          </div>
        </div>
      </Link>

      <div className="flex items-center gap-3 text-sm md:text-base">
        <span className="font-medium text-black">{project.category}</span>
        <span className="font-light italic text-gray-400">pour</span>
        <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-black"></div>
            <span className="font-bold text-black">{project.client}</span>
        </div>
      </div>
    </div>
  )
}

// --- COMPOSANT PRINCIPAL (C'est lui qu'on importe) ---
export default function TrustedSection() {
  return (
    <section className="relative w-full bg-[#FFFFFF] py-20 md:py-32"> 
      <div className="container mx-auto px-5 md:px-10">
        
        <div className="grid grid-cols-1 md:grid-cols-12 mb-16">
            <div className="col-span-1 md:col-span-6 lg:col-span-5">
                <h2
                    className="text-black leading-[1.1]"
                    style={{
                        fontSize: 'clamp(32px, 4vw, 55px)',
                        fontFamily: 'Helvetica Now Display, Arial, sans-serif',
                        fontWeight: '400'
                    }}
                >
                    Parce qu'ils nous ont déjà fait confiance
                </h2>
            </div>

            <div className="hidden md:flex col-span-6 lg:col-span-7 justify-end items-start">
                 <Link href="/works" className="px-6 py-3 rounded-full border border-black/10 hover:bg-black hover:text-white transition-colors text-black">
                    Voir les projets →
                 </Link>
            </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-16">
          {projects.map((project) => (
            <ProjectCard key={project.id} project={project} />
          ))}
        </div>

      </div>
    </section>
  )
}